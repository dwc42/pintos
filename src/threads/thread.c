...
bool sortByPiority(const struct list_elem *a,
...
static void print_thread_elem(struct list_elem *e, void *aux UNUSED)
{
  struct thread *t = list_entry(e, struct thread, elem);
  printf("READY: tid=%d pri=%d name=%s\n", t->tid, t->priority, t->name);
}
void debug_print_ready_list(void)
{
  enum intr_level old_level = intr_disable();
  list_for_each(&ready_list, print_thread_elem, NULL);
...
void thread_unblock(struct thread *t)
{
  ...
  list_insert_ordered(&ready_list, &t->elem, sortByPiority, NULL);
  t->status = THREAD_READY;
  // printf("unblock: tid=%d pri=%d cur=%d pri=%d\n",t->tid, t->priority, thread_current()->tid, thread_current()->priority);
  intr_set_level(old_level);
}
